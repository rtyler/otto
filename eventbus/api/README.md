# Rust API for eventbus-api

This specification describes the Otto event bus
...
- API version: 1.0.0
- Build date: 2019-05-25T04:25:02.953Z
...
It also contains an example server and client which make use of `eventbus-api`:
* The example server starts up a web server using the `eventbus-api` router,
...
* The example client provides a CLI which lets you invoke any single operation on the
  `eventbus-api` client by passing appropriate arguments on the command line.
...
```
cargo run --example client ChannelNameGet
cargo run --example client ChannelNameOffsetGet
cargo run --example client ChannelNamePatch
cargo run --example client ChannelNamePost
cargo run --example client ChannelNamePut
cargo run --example client ListChannels
cargo run --example client OffsetConsumerGet
cargo run --example client OffsetConsumerPatch
cargo run --example client OffsetConsumerPost
```
...
* Insert `eventbus-api` into the `members` array under [workspace] in the root `Cargo.toml`, e.g., `members = [ "eventbus-api" ]`.
* Add `eventbus-api = {version = "1.0.0", path = "eventbus-api"}` under `[dependencies]` in the root `Cargo.toml`.
* Copy the `[dependencies]` and `[dev-dependencies]` from `eventbus-api/Cargo.toml` into the root `Cargo.toml`'s `[dependencies]` section.
...
```
cp eventbus-api/examples/server.rs src/main.rs
cp eventbus-api/examples/server_lib/mod.rs src/lib.rs
cp eventbus-api/examples/server_lib/server.rs src/server.rs
```
